...
/* Date de création :  13/10/2021 22:54:40                      */
/*==============================================================*/
...
if exists(select 1 from sys.sysforeignkey where role='FK_CANDIDAT_ASSOCIATI_CANDIDAT') then
    alter table Candidature
       delete foreign key FK_CANDIDAT_ASSOCIATI_CANDIDAT
end if;

if exists(select 1 from sys.sysforeignkey where role='FK_CANDIDAT_ASSOCIATI_OFFRE') then
    alter table Candidature
       delete foreign key FK_CANDIDAT_ASSOCIATI_OFFRE
end if;
...
if exists(select 1 from sys.sysforeignkey where role='FK_LINK_LINK_ENTREPRI') then
    alter table link
       delete foreign key FK_LINK_LINK_ENTREPRI
end if;

if exists(select 1 from sys.sysforeignkey where role='FK_LINK_LINK_RECRUTEU') then
    alter table link
       delete foreign key FK_LINK_LINK_RECRUTEU
end if;
...
if exists(
   ...
   where i.table_id=t.table_id 
     ...
     and t.table_name='Candidature'
) then
   drop index Candidature.ASSOCIATION1_FK2
end if;
...
if exists(
   ...
   drop index Candidature.ASSOCIATION1_FK
end if;

if exists(
   select 1 from sys.sysindex i, sys.systable t
   where i.table_id=t.table_id 
     and i.index_name='CANDIDATURE_PK'
     and t.table_name='Candidature'
) then
   drop index Candidature.CANDIDATURE_PK
end if;
...
if exists(
   select 1 from sys.sysindex i, sys.systable t
   where i.table_id=t.table_id 
     and i.index_name='ENTREPRISE_PK'
     and t.table_name='Entreprise'
) then
   drop index Entreprise.ENTREPRISE_PK
end if;
...
if exists(
   select 1 from sys.sysindex i, sys.systable t
   where i.table_id=t.table_id 
     and i.index_name='LINK_FK2'
     and t.table_name='link'
) then
   drop index link.LINK_FK2
end if;

if exists(
   select 1 from sys.sysindex i, sys.systable t
   where i.table_id=t.table_id 
     and i.index_name='LINK_FK'
     and t.table_name='link'
) then
   drop index link.LINK_FK
end if;

if exists(
   select 1 from sys.sysindex i, sys.systable t
   where i.table_id=t.table_id 
     and i.index_name='LINK_PK'
     and t.table_name='link'
) then
   drop index link.LINK_PK
end if;
...
(
   ...
   cv                   varchar(254)                   null,
   disponibilite        varchar(254)                   null,
   fonctionPrincipale   varchar(254)                   null,
...
(
   ...
   idRecruteur          integer                        not null,
   idEntreprise         integer                        not null,
   constraint PK_LINK primary key clustered (idMembre, idRecruteur, idEntreprise)
);

/*==============================================================*/
/* Index : LINK_PK                                              */
/*==============================================================*/
create unique clustered index LINK_PK on link (
idMembre ASC,
idRecruteur ASC,
idEntreprise ASC
);
...
alter table link
   add constraint FK_LINK_LINK_ENTREPRI foreign key (idEntreprise)
      references Entreprise (idEntreprise)
      on update restrict
      on delete restrict;

alter table link
   add constraint FK_LINK_LINK_RECRUTEU foreign key (idMembre, idRecruteur)
      references Recruteur (idMembre, idRecruteur)
      on update restrict
      on delete restrict;
